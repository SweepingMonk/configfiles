...
function VQ2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  if (server.indexOf("obfs=") == -1) { // 非 ws/http 类型
    ...
    var path = server.indexOf("obfs-path=") != -1 ? server.split("obfs-path=")[1].split("\"")[1] : "/"
    var uri = path ? "obfs-uri=" + path + ", " : ""
    obfs = obfs + uri
...
function VL2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  if (cnt.indexOf("path=") != -1) {
    var path = decodeURIComponent(cnt.split("path=")[1].split("&")[0].split("#")[0])
    puri = path ? "obfs-uri=" + path : ""
  }
...
function LoonVL2QX(cnt) {
  ...
  vpath = cnt.indexOf("path=")==-1? "":cnt.split("path=")[1].split(",")[0]
  vpath = vpath ? "obfs-uri="+vpath : ""
...
function CV2QX(cnt) {
  ...
  ouri = cnt["ws-opts"] && "path" in cnt["ws-opts"] ? (cnt["ws-opts"]["path"] ? "obfs-uri="+cnt["ws-opts"]["path"] : "") : ouri
